...
<div class="form-inline">
    <div class="form-group">
        <ul class="pagination">
        	...
        	<fmt:formatNumber type="number"  value="${((page.pageNo-1)-(page.pageNo-1)%pageLen)/pageLen}"  var="pageIndex"/>
            <c:choose>
                <c:when test="${page.totalPage > 1}">
                    ...
                    <c:forEach var="p" begin="${pageLen*pageIndex + 1}" end="${lastPageNo gt page.totalPage ? page.totalPage:lastPageNo}" step="1">
                        <c:choose>
                            <c:when test="${p == page.pageNo}">
                                <li class="active"><a href="javascript:">${p}<span class="sr-only">(current)</span></a></li>
                            </c:when>
                            <c:otherwise>
    ...
    <div class="form-group font-size-0">
        ...
        <div class="form-group"><button id="goPage" class="btn btn-danger vcard-btn-page btn-sm">GO</button></div>
    </div>
