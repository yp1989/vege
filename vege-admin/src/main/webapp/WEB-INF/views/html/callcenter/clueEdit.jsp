...
<html lang="en">
	...
	<body class="no-skin">
				<form class="form-horizontal" role="form" method="post" id="addNewCustForm" action="/callcenter/doAddClue">
				<div class="page-content">
					...
					<div class="row">
						<div class="col-md-12">
								...
								<div class="form-group">
									<label class="col-md-1 control-label" for="form-field-1">客户名称<span class="required-star">*</span>：</label>
									<div class="col-md-6">
										<input type="text" id="form-field-1" name="clue.custName" class="input-sm form-control" value="${clue.custName}">
									</div>
								</div>
								<div class="form-group">
									<label class="col-md-1 control-label" for="form-field-1">客户地址<span class="required-star">*</span>：</label>
									<div class="col-md-2">
								...
								<div class="form-group">
									...
									<div class="col-md-6">
										<input type="text" id="form-field-2" class="input-sm form-control" name="clue.custAddr" value="${clue.custAddr}">
									</div>
								</div>
                                        ...
                                        <td>
                                            <select class="form-control" name="customerList[${index.index}].custRole">
                                                <c:forEach items="${roles}" var="r">
                                                    <option value="${r.code}" <c:if test="${c.custRole == r.code}">selected</c:if>>${r.desc}</option>
                                                </c:forEach>
                                            </select>
                                        </td>
                                        <td><input type="text" class="form-control" name="customerList[${index.index}].custName" value="${c.custName}" placeholder="请输入姓名"></td>
                                        <td><input type="text" class="form-control" name="customerList[${index.index}].custRole" value="${c.custJob}" placeholder="请输入职务"></td>
                                        <td><input type="text" class="form-control" name="customerList[${index.index}].custPhone" value="${c.custPhone}" placeholder="请输入联系方式"></td>
                                        <td><input type="text" class="form-control" name="customerList[${index.index}].custQq" value="${c.custQq}" placeholder="请输入QQ"></td>
                                        <td><input type="text" class="form-control" name="customerList[${index.index}].custEmail" value="${c.custEmail}" placeholder="请输入邮箱"></td>
					...
					<div class="mt20">
						...
						<div class="row">
							<div class="col-md-12">
									...
									<div class="form-group">
										<label class="col-md-1 control-label" for="form-field-1">线索来源<span class="required-star">*</span>：</label>
										<div class="col-md-2">
											<select class="input-sm form-control" name="clue.clueSource" id="clueSource">
                                                ...
                                                <c:forEach items="${ClueSource}" var="c" varStatus="index">
                                                    <option value="${c.code}" <c:if test="${clue.clueSource == c.code}">selected</c:if>>${c.desc}</option>
                                                </c:forEach>
											</select>
										</div>
									</div>
									<div class="form-group">
										<label class="col-md-1 control-label" for="form-field-1">了解方式<span class="required-star">*</span>：</label>
										<div class="col-md-2">
											<select class="input-sm form-control" name="clue.clueKnow" id="clueKnow">
												<option value="">请选择</option>
									...
									<div class="form-group">
										<label class="col-md-1 control-label" for="form-field-1">咨询方式<span class="required-star">*</span>：</label>
										<div class="col-md-2">
											<select class="input-sm form-control" name="clue.clueConsult" id="clueConsult">
												<option value="">请选择</option>
                                                <c:forEach items="${ClueConsult}" var="c" varStatus="index">
                                                    <option value="${c.code}" <c:if test="${clue.clueConsult == c.code}">selected</c:if> >${c.desc}</option>
                                                </c:forEach>
											</select>
										</div>
									</div>
									<div class="form-group">
										<label class="col-md-1 control-label" for="form-field-1">线索类型<span class="required-star">*</span>：</label>
										<div class="col-md-2">
									...
									<div class="form-group">
										<label class="col-md-1 control-label" for="form-field-1">预购产品<span class="required-star">*</span>：</label>
										<div class="col-md-10">
											<c:forEach items="${products}" var="c" varStatus="index">
                                                    <label class="checkbox-inline no-padding-left">
                                                        <input name="productIds" type="checkbox" value="${c.id}" class="ace" <c:forEach items="${clueProducts}" var="cp" ><c:if test="${c.id == cp.id}">checked</c:if></c:forEach>>
									...
									<div class="form-group">
										...
										<div class="col-md-5">
											<input type="text" name="clue.clueKeyword" id="form-field-3" class="input-sm form-control" value="${clue.clueKeyword}">
										</div>
									</div>
									<div class="form-group">
										<label class="col-md-1 control-label" for="form-field-1">备注：</label>
										<div class="col-md-5">
											<textarea id="form-field-11" class="input-sm form-control" name="clue.remark" style="overflow: hidden; word-wrap: break-word; resize: horizontal; height: 52px;">${clue.remark}</textarea>
										</div>
									</div>
						...
						<form role="form">
							...
							<div class="form-group no-margin">
								...
								<div class="input-group col-md-2">
									<input type="text" data-date-format="dd-mm-yyyy" id="id-date-picker-1" class="form-control date-picker" name="callback.callNextTime">
										...
										<div class="widget-main padding-4">
											<div class="tab-content padding-8">
												<div id="task-tab" class="tab-pane active">
													<table id="vweddingTable1" class="table table-striped table-bordered table-hover dataTable ">
									                    <thead>
										                    <tr>
										                        <th>回访人</th>
										                        <th>回访时间</th>
                                                        ...
                                                        <c:forEach items="${callbackList}" var="c" varStatus="index">
                                                            <tr>
                                                                <td>${c.adminName}</td>
                                                                <td><fmt:formatDate value="${c.callTime}" pattern="yyyy-MM-dd"/></td>
                                                                <td>${c.callBargainStr}</td>
                                                                <td><fmt:formatDate value="${c.callNextTime}" pattern="yyyy-MM-dd"/></td>
                                                                <td>${c.callContent}</td>
                                                            </tr>
                                                        </c:forEach>
									                    </tbody>
												...
												<div id="member-tab" class="tab-pane">
                                                    <c:forEach items="${allocationLogList}" var="c">
                                                        <p class="muted"><fmt:formatDate value="${c.createTime}" pattern="yyyy年MM月dd日 HH:mm"/> 由   ${c.alloAdminName} 分配给   ${c.alloOwnerName}</p>
                                                    </c:forEach>
												</div>
												<div id="comment-tab" class="tab-pane">
													<table id="vweddingTable2" class="table table-striped table-bordered table-hover dataTable ">
									                    <thead>
										                    <tr>
										                        <th>操作时间</th>
										                        <th>操作人</th>
                    ...
                    <div class="form-group">
                        <div class="col-md-5">
							...
							<input type="hidden" name="clue.id" value="${clue.id}">
                        </div>
                    </div>
				</div>
					<!--该号码关联线索-->
