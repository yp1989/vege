            ...
            <form class="form-horizontal" role="form" action="/contract/contractPage" method="post" id="findForm">
                ...
                <div class="col-md-3 pb10 no-padding-left">
                    <div class="input-group">
                        ...
                        <input type="text" name="cust" placeholder="请输入客户名称" value="${contract.cust}"
                ...
                <div class="col-md-3 pb10 no-padding-left">
                    <div class="input-group">
                        ...
                        <div class="input-daterange input-group">
                            <input type="text" class="input-sm form-control" name="createStartTime"
                                   value='<fmt:formatDate value="${contract.createStartTime}" pattern="yyyy-MM-dd"/>'>
                            <span class="input-group-addon">
									<i class="fa fa-exchange"></i>
								</span>

                            <input type="text" class="input-sm form-control" name="createEndTime"
                                   value='<fmt:formatDate value="${contract.createEndTime}" pattern="yyyy-MM-dd"/>'>
                        </div>
                    </div>
                </div>
                <div class="col-md-3 pb10 no-padding-left">
                        ...
                        <select class="input-sm form-control" name="contStatus">
                            ...
                            <c:forEach items="${statusEnum}" var="st" varStatus="index">
                                <option value="${st.code}"
                                        <c:if test="${contract.contStatus == st.code}">selected</c:if>>${st.desc}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
                <div class="col-md-3 pb10 no-padding-left">
                    <div class="input-group">
                        ...
                        <input type="text" name="ownerName" placeholder="请输入归属人姓名" value="${contract.ownerName}"
                               class="input-sm form-control">
                    </div>
                </div>
                <div class="col-md-3 pb10 no-padding-left">
                    <div class="input-group">
                        ...
                        <div class="selected_box">
                            ...
                            <input type="text" readonly="readonly" id="versShowName" class="pl5 package_btn"
                                   value="请选择">
                            <div class="select_ul">
                                <ul>
                    ...
                    <div class="input-group">
                        ...
                        <select class="input-sm form-control" name="uncollected">
                            ...
                            <c:forEach items="${unGatheringEnum}" var="unGathering" varStatus="index">
                                <option value="${unGathering.code}"
                                        <c:if test="${contract.uncollected == unGathering.code}">selected</c:if>>${unGathering.desc}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
                        ...
                        <select class="input-sm form-control" name="contType">
                            ...
                            <c:forEach items="${typeEnum}" var="type" varStatus="index">
                                <option value="${type.code}"
                                        <c:if test="${contract.contType == type.code}">selected</c:if>>${type.desc}</option>
                            </c:forEach>
                        </select>
                    </div>
            ...
            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                ...
                <c:forEach var="contract" items="${page.results}" varStatus="index">
                    <tr class="selected">
                        ...
                        <td>
                            <a class="mr20" onclick="checkContract('${contract.id}');" data-toggle="modal"
                               data-target="#contractInfo">查看</a>
                            <!-- 合同审核中 -->
                            <c:if test="${contract.contStatus == 1}">
                                <a name="checkBtn" class="mr20" data-toggle="modal" data-target="#modal_checkClose"
                                   style="display: none;" onclick="audit('${contract.id}','${contract.busiId}');">审核</a>
                                <a name="cancelBtn" onclick="contractCancel('${contract.id}');" style="display: none;"
                                   class="mr20">作废</a>
                            </c:if>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default btn-sm" data-dismiss="modal" onclick="cancelCheck();">关闭
                </button>
            </div>
        </div>
    </div>
...
<!--审核-->
<div class="modal fade" id="modal_checkClose" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="contractCheckForm" method="post">
                ...
                <div class="modal_more">
                    ...
                    <textarea id="form-field-11" class="autosize-transition form-control"
                              name="returnChangeRemark"></textarea>
                    <div class="text-right">
                        <button type="button" class="btn btn-primary btn-sm mt10" data-dismiss="modal"
                                onclick="makeSurePass(4);">提交
                        </button>
                    </div>
                </div>
            </form>
