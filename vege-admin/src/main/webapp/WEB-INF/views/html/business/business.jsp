	...
	<body class="no-skin">
				<div class="page-content">
					...
					<div class="row">
							...
							<form class="form-horizontal" role="form" action="/business/index" method="post" id="findForm">
								...
								<div class="col-md-3 pb10 no-padding-left">
									<div class="input-group">
										...
										<input type="text" name="custName" placeholder="请输入客户名称" value="${business.custName}" class="input-sm form-control">
									</div>
								</div>
								<div class="col-md-3 pb10 no-padding-left">
									<div class="input-group">
										...
										<div class="input-daterange input-group">
											<input type="text" class="input-sm form-control" name="createTimestart" value='<fmt:formatDate value="${business.createTimestart}" pattern="yyyy-MM-dd"/>'>
											<span class="input-group-addon">
												<i class="fa fa-exchange"></i>
											</span>

											<input type="text" class="input-sm form-control" name="createTimeend" value='<fmt:formatDate value="${business.createTimeend}" pattern="yyyy-MM-dd"/>'>
										</div>
									</div>
								</div>
								<div class="col-md-3 pb10 no-padding-left">
									<div class="input-group">
										...
										<select class="input-sm form-control" name="busiStatus" id="busiStatus_id">
											...
											<c:forEach items="${businessStatus}" var="c" varStatus="index">
												<option value="${c.code}" <c:if test="${busi.busiStatus == c.code}">selected</c:if>>${c.desc}</option>
											</c:forEach>
										</select>
									</div>
								</div>
									...
									<div class="input-group">
										...
										<select class="input-sm form-control" name="busiSource" id="busiSource_id">
											...
											<c:forEach items="${businessSource}" var="c" varStatus="index">
												<option value="${c.code}" <c:if test="${busi.busiSource == c.code}">selected</c:if>>${c.desc}</option>
											</c:forEach>
										</select>
									</div>
								</div>
									...
									<div class="input-group">
										...
										<select class="input-sm form-control" name="owner" id="form-field-owner">
											...
											<c:forEach items="${adminList}" var="c" varStatus="index">
												<option value="${c.id}" <c:if test="${busi.owner == c.id}">selected</c:if> >${c.adminRealName}</option>
											</c:forEach>
										</select>
									</div>
								</div>
									...
									<div class="input-group">
										...
										<select class="input-sm form-control" name="busiType">
											...
											<c:forEach items="${businessType}" var="c" varStatus="index">
												<option value="${c.code}" <c:if test="${busi.busiType == c.code}">selected</c:if> >${c.desc}</option>
											</c:forEach>
										</select>
									</div>
								</div>
									...
									<div class="input-group">
										...
										<select class="input-sm form-control" name="callBargain" id="dealIntention">
											...
											<c:forEach items="${productVersions}" var="c" varStatus="index">
												<option value="${c.id}" <c:if test="${clue.callBargain == c.id}">selected</c:if> >${c.versName}</option>
											</c:forEach>
										</select>
									</div>
								</div>
                        ...
                        %>
						<div class="col-md-12">
							<a href="####" data-toggle="modal" data-target="#myModal" id="allotClue_id" class="btn btn-primary btn-sm btn-filter">商机分配</a>
						</div>
                        <%}%>
							...
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<c:forEach items="${page.results}" var="p" varStatus="index">
									<tr class="selected">
										...
										<td>${p.custName}</td>
										<td>${p.busiStatusStr}<c:if test="${p.busiStatus == 2}"><a href="###" data-toggle="modal" id="return_clue_detail" data-target="#modal_business_detail" data-remark="${p.returnChangeRemark}">详情</a></c:if></td>
										...
										<td>
											...
											<c:if test="${p.busiStatus == 1}">
												<a href="###" data-toggle="modal" data-target="#Auditing" data-id="${p.id}" class="mr20" id="auditing_shenhe">审核</a>
											</c:if>
                                            <%}%>
                                            <c:if test="${p.busiStatus == 3 || p.busiStatus == 2}">
												<c:if test="${p.busiType == 1}">
													<a href="/business/productsContract?busiId=${p.id}"  data-id="${p.id}" class="mr20" id="auditing_shenhe">生成合同</a>
												</c:if>
												<c:if test="${p.busiType == 2}">
													<a href="/business/agentContract?busiId=${p.id}"  data-id="${p.id}" class="mr20" id="auditing_shenhe">生成合同</a>
												</c:if>
											</c:if>
							...
							<div class="modal-content">
								...
								<div class="modal_more hide">
									...
									<div class="text-right">
										<button type="button" class="btn btn-primary btn-sm mt10 auditing-mt10 sure_submit" data-dismiss="modal">提交</button>
									</div>
								</div>
							</div><!-- /.modal-content -->
						</form>
                ...
                <!--详情-->
                <div class="modal fade" id="modal_business_detail" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-body">
