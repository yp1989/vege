                                                        ...
                                                        <div class="form_group pb10">
                                                            ...
                                                            <c:forEach items="${p.versionList}" var="v">
                                                                <label class="checkbox-inline no-padding-left">
                                                                    <input name="versionIds" type="checkbox" class="ace products" value="${v.id}" data-productType="${p.productType}" data-pro-id="${p.id}" data-standardAmount="${v.versPrice}" data-name="${v.versName}" <c:forEach items="${business.productVersionList}" var="bp"><c:if test="${v.id == bp.id}">checked</c:if></c:forEach> >
													...
													<table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                        ...
                                                        <c:forEach items="${business.productVersionList}" var="bp" varStatus="index">
															<tr class="selected" data-v-id="${bp.id}">
																<td>${bp.versName}
																	<input type="hidden" name="contProducts[${index.index}].prodId" value="${bp.prodId}"/>
																	<input type="hidden" name="contProducts[${index.index}].versId" value="${bp.id}"/>
																</td>
																<td class="count_num"><input type="text"  name="contProducts[${index.index}].buyCount" class="col-xs-10 col-sm-5" value=""></td>
																<td class="Standard_amount">${bp.versPrice}<input type="hidden" name=""/></td>
																<td class="Actual_amount"><input type="text"  name="contProducts[${index.index}].actualAmount" class="col-xs-10 col-sm-5" value=""></td>
																<td class="zong_ren_tian"><input type="text"  name="contProducts[${index.index}].sumMenday" class="col-xs-10 col-sm-5" value=""></td>
																<td class="ren_tian_dan_jian"><input type="text"  name="contProducts[${index.index}].priceMenday" class="col-xs-10 col-sm-5" value=""></td>
																<td class="Discount"><input type="text"  name="contProducts[${index.index}].prodDiscount" class="col-xs-10 col-sm-5" value=""></td>
															</tr>
                                                        </c:forEach>
                                                        <tr class="heji">
                                                            <td>合计</td>
                                                            ...
                                                            <td class="Actual_amount">0<input type="hidden" name="contract.contReceivable" value=""/></td>
