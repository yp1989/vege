...
<html lang="en">
	<head>
		...
		<!-- css样式表异步加载 在/layout/stylesheets.html内 -->
							...
							<form class="form-horizontal" role="form" action="/clue/index" method="post" id="findForm">
								...
								<div class="col-md-3 pb10 no-padding-left">
									<div class="input-group">
										...
										<input type="text" placeholder="请输入QQ" name="contactsQQ" value="${clue.contactsQQ}"  class="input-sm form-control">
									</div>
								</div>
								<div class="col-md-3 pb10 no-padding-left">
									<div class="input-group">
                                ...
                                <div class="col-md-3 pb10 no-padding-left">
                                    <div class="input-group">
                                        ...
                                        <select class="input-sm form-control" name="optionStatus">
                                            <option value="">全部</option>
                                            <option value="0" <c:if test="${clue.optionStatus == 0}">selected</c:if> >未回访</option>
                                            <option value="1" <c:if test="${clue.optionStatus > 0}">selected</c:if> >已回访</option>
                                        </select>
                                    </div>
                                </div>
								<div class="col-md-3 pb10 no-padding-left">
									<div class="input-group">
										...
										<div class="input-daterange input-group">
											<input type="text" class="input-sm form-control" name="callTimestart" value='<fmt:formatDate value="${clue.callTimestart}" pattern="yyyy-MM-dd"/>'>
											<span class="input-group-addon">
												<i class="fa fa-exchange"></i>
											</span>

											<input type="text" class="input-sm form-control" name="callTimeend" value='<fmt:formatDate value="${clue.callTimeend}" pattern="yyyy-MM-dd"/>'>
										</div>
									</div>
								</div>
								<div class="col-md-3 pb10 no-padding-left">
									<div class="input-group">
										...
										<div class="input-daterange input-group">
											<input type="text" class="input-sm form-control" name="nextTimestart" value='<fmt:formatDate value="${clue.nextTimestart}" pattern="yyyy-MM-dd"/>'>
											<span class="input-group-addon">
												<i class="fa fa-exchange"></i>
											</span>

											<input type="text" class="input-sm form-control" name="nextTimeend" value='<fmt:formatDate value="${clue.nextTimeend}" pattern="yyyy-MM-dd"/>'>
										</div>
									</div>
								</div>
							...
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<c:forEach items="${page.results}" var="p" varStatus="index">
									<tr class="selected <c:if test="${p.isRepeat > 0}">red</c:if>">
										<td class="center">
											<label class="position-relative">
												<input type="checkbox" class="ace acechild" value="${p.id}" data-status="${p.clueStatusOnline}" name="clueIds">
												<span class="lbl"></span>
											</label>
										</td>
										...
										<td>${p.clueStatusOnlineStr}&nbsp;&nbsp;<c:if test="${p.clueStatusOnline == 3}"><a href="###" data-toggle="modal" id="return_clue_detail" data-target="#modal_clue_detail" data-remark="${p.returnChangeRemark}">详情</a></c:if></td>
										...
										<td>
											...
											<c:if test="${p.clueStatusOnline != 2 && p.clueStatusOnline != 0 && p.clueStatusOnline != 4}">
												<a href="javascript:;" class="mr20" data-toggle="modal" data-target="#modal_generate" data-clue-type="${p.clueType}" data-id="${p.id}" id="modal_generate_id">生成商机</a>
											</c:if>
												<c:if test="${p.clueStatusOnline != 2 && p.clueStatusOnline != 4}">
				...
				<!--生成商机-->
				<div class="modal fade" id="modal_generate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false">
				...
				<!--详情-->
				<div class="modal fade" id="modal_clue_detail" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal"
